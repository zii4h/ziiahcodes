...
const getImgUrl = (activity, key) => {
  ...
  if (v.startsWith("spotify:"))
    return `https://i.scdn.co/image/${v.replace("spotify:", "")}`;
  if (v.startsWith("mp:external/")) {
    const path = v.replace("mp:external/", "").split("/").slice(1).join("/");
    try {
      return "https://" + decodeURIComponent(path.replace(/^https?\//, ""));
    } catch {
      return "";
    }
  }
  if (v.startsWith("mp:"))
    return `https://media.discordapp.net/${v.replace("mp:", "")}`;
...
const ActivityCard = ({ activity }) => {
  ...
  const elapsed =
    !isSpotify && activity.timestamps?.start
      ? getElapsed(activity.timestamps.start)
      : null;
  ...
  const spotify =
    isSpotify && activity.timestamps?.start && activity.timestamps?.end
      ? (() => {
          ...
          return {
            cur: formatTime(Math.min(el, dur)),
  ...
  <div style={{ position: "relative", flexShrink: 0 }}>
    <img src={largeImg} alt="" width={62} height={64}
      ...
      >
        ...
        {elapsed && (
          <div className="activity-detail" style={{ fontSize: 10, color: "var(--text3)" }}>
            {elapsed}
          </div>
        )}
      </div>
    ...
    return (
      <div style={cardStyle}>
        ...
        <div style={{ padding: "0 12px 12px" }}>
          ...
          />
          <div
            style={{ height: 8, width: "40%", background: "var(--bg3)", borderRadius: 4 }}
          />
        </div>
      </div>
    );
